...
-- MAGIC      - Defina seu esquema **labuser** como o esquema default. Ao definir o esquema default, use a cláusula `IDENTIFIER` com a variável `DA.schema_name` para definir seu esquema.
-- MAGIC
...
-- MAGIC
-- MAGIC     **DICAS:**
-- MAGIC       - [consultar os dados por caminho](https://docs.databricks.com/aws/en/query#query-data-by-path)  para visualizar os arquivos JSON brutos usando `json`.
-- MAGIC       - Adicione uma opção `LIMIT` para limitar os resultados a 10 linhas.
...
-- MAGIC     - Crie uma tabela chamada **customers_lab** no esquema **dbacademy.labuser**.
-- MAGIC
...
-- MAGIC
-- MAGIC - [read_files função com valor de tabela](https://docs.databricks.com/aws/en/sql/language-manual/functions/read_files)
-- MAGIC
