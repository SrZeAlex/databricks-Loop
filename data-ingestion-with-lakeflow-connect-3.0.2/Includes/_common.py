...
schema_name = user.split("@")[0].replace(".", "_")

spark.sql(f"CREATE SCHEMA IF NOT EXISTS {schema_name}")
spark.sql(f"CREATE VOLUME IF NOT EXISTS databrickstheloop.{schema_name}.{schema_name}")

# COMMAND ----------

spark.sql(f"""CREATE OR REPLACE TABLE databrickstheloop.{schema_name}.meta (
    owner STRING,
    object STRING,
    key STRING,
    value STRING
...
spark.sql(f"""
INSERT INTO databrickstheloop.{schema_name}.meta (owner, object, key, value)
VALUES
    (current_user(), NULL, 'username', current_user()),
    (current_user(), NULL, 'catalog_name', 'databrickstheloop'),
    (current_user(), NULL, 'schema_name', '{schema_name}'),
    (current_user(), NULL, 'paths.working_dir', '/Volumes/databrickstheloop/{schema_name}/{schema_name}'),
    ...
    (current_user(), NULL, 'pseudonym', '{schema_name}'),
    (current_user(), NULL, 'cluster_name', 'DataModelingDBX')
    """)
...
class DBAcademyHelper(NestedNamespace):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            default_catalog = 'databrickstheloop'

        user = spark.sql("SELECT current_user()").collect()[0][0]
        schema_name = user.split("@")[0].replace(".", "_")

        meta = f'{default_catalog}.{schema_name}.meta'
        catalog = None
        schema = None
